...
namespace prx
{
    ...
    {     
        ...
        {
            PRX_FATAL_S("Cannot remove points from the ANN distance metric!");
        }
        ...
        const abstract_node_t* ann_distance_metric_t::single_query(const space_point_t* query_point, double* dist) const
        {
            if(dist!=NULL)
                PRX_FATAL_S("Ann metric can't populate dist parameter in single query because reasons.");
            if( nr_points == 0 )
                return NULL;
