...
#include "planning/task_planners/simple_place_tp.hpp"
...
PLUGINLIB_EXPORT_CLASS(prx::packages::manipulation::simple_place_tp_t ,prx::plan::planner_t)
...
namespace prx
{
    ...
    namespace packages
    {
        namespace manipulation
        {
            ...
            simple_place_tp_t::simple_place_tp_t()
            {
            ...
            void simple_place_tp_t::init(const parameter_reader_t* reader, const parameter_reader_t* template_reader)
            {
                PRX_DEBUG_COLOR("Initializing simple_place_tp_t task planner ...", PRX_TEXT_CYAN);
            ...
            void simple_place_tp_t::link_world_model(world_model_t * const model)
            {
            ...
            void simple_place_tp_t::link_query(query_t* new_query)
            {
                task_planner_t::link_query(new_query);
                in_query = static_cast<motion_planning_query_t*>(new_query);
            }

            void simple_place_tp_t::setup()
            {
                manipulation_model->use_context("full_space");
                manip_initial_state = manipulation_model->get_state_space()->alloc_point();
                PRX_PRINT ("Manipulation model initial state: " <<  manipulation_model->get_state_space()->print_point(manip_initial_state,2), PRX_TEXT_LIGHTGRAY);
            ...
            bool simple_place_tp_t::execute()
            {
                foreach(planner_t* planner, planners | boost::adaptors::map_values)
                {                    
            ...
            bool simple_place_tp_t::succeeded() const
            {
                return true;
            }

            const util::statistics_t* simple_place_tp_t::get_statistics()
            {
                return NULL;
            }

            void simple_place_tp_t::resolve_query()
            {
                config_t retract_config;
                retract_config.set_position(0,0,-.05);
                retract_config.set_orientation(0,0,0,1);
                PRX_DEBUG_COLOR("Resolve query from simple_place_tp ...  context:" << manipulation_context_name,PRX_TEXT_GREEN);
                manipulation_model->use_context("full_space");
                ...
                state_t* target_state = manipulation_model->get_state_space()->alloc_point();

                state_t* initial_object = object_space->alloc_point();
                state_t* target_object = object_space->alloc_point();
                quaternion_t target_quat(object_target_vec[3],object_target_vec[4],object_target_vec[5],object_target_vec[6]);
                target_quat.rectify();
                target_quat.get(object_target_vec[3],object_target_vec[4],object_target_vec[5],object_target_vec[6]);
                
                object_space->copy_vector_to_point(object_target_vec, target_object);
                
                //object_space->copy_from_point(target_object);
                PRX_PRINT ("PRE SETUP object_target_vec: " << object_space->print_point(target_object), PRX_TEXT_BLUE);
                PRX_PRINT ("PRE SETUP INITIAL OBJECT STATE: " << object_space->print_point(initial_object), PRX_TEXT_BLUE);
                manipulation_query = dynamic_cast< manipulation_query_t* >( output_queries[ manipulation_task_planner_name ] );
                PRX_PRINT("Manipulation query initialized with mode; " << manipulation_query->astar_mode, PRX_TEXT_CYAN);

                //PLACE VARIABLES
                grasp_t grasp;
                config_t ee_config, object_config;

                manipulation_model->get_current_manipulation_info()->manipulator->get_end_effector_configuration( ee_config, 0 );
                object_config.set_position( initial_object->memory[0], initial_object->memory[1], initial_object->memory[2] );
                object_config.set_orientation( initial_object->memory[3], initial_object->memory[4], initial_object->memory[5], initial_object->memory[6] );
                
                manipulation_model->FK(ee_config);
                grasp.relative_config = ee_config;
                grasp.relative_config.global_to_relative(  object_config);
                
                PRX_DEBUG_COLOR("Grasp relative config: " << grasp.relative_config.print(), PRX_TEXT_CYAN);
                grasp.release_mode = 1;
                grasp.grasping_mode = 2;
                //MOVE VARIABLES
                target_state->memory[1] = 2.86;
                target_state->memory[3] = 0.8;
                target_state->memory[4] = 1.6;
                target_state->memory[5] = 0.3;

                // PRX_PRINT ("POST SETUP object_target_vec: " << object_space->print_point(target_object), PRX_TEXT_BLUE);

                //Let's try just a move
                // manipulation_query->setup_move( manipulation_context_name, initial_state, target_state );

                //Place the thing
                manipulation_query->setup_place(manipulation_context_name, true, move_object, 1, retract_config, initial_state, target_object, &grasp);

                //Or a place and retract
                //manipulation_query->setup_place(manipulation_context_name, true, move_object, 1, retract_config, initial_state, target_object, &grasp);



                simple_place_specification_t* place_spec = dynamic_cast< simple_place_specification_t* >(input_specification);
                manipulation_query->path_constraints = place_spec->validity_checker->alloc_constraint();
                constraints_t* valid_constraints = place_spec->validity_checker->alloc_constraint();
                ...
                if( occ != NULL )
                {
                    PRX_PRINT("PLACE TP: Cast to Object Collision Constraints worked!", PRX_TEXT_GREEN);
                ...
                {
                    PRX_PRINT("PLACE TP: Cast to Object Collision Constraints DID NOT WORK!", PRX_TEXT_RED);                    
                }
...
//                PRX_PRINT ("PRE  RESOLVE object_target_vec: " << object_space->print_point(target_object), PRX_TEXT_BLUE);
                planners[manipulation_task_planner_name]->resolve_query();

//                PRX_PRINT ("POST RESOLVE object_target_vec: " << object_space->print_point(target_object), PRX_TEXT_BLUE);
                ...
                PRX_PRINT("New object state: " << object_space->print_memory(4), PRX_TEXT_CYAN);
