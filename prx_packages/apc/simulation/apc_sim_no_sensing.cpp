...
#include "simulation/apc_sim_no_sensing.hpp"
...
PLUGINLIB_EXPORT_CLASS(prx::packages::apc::apc_sim_no_sensing_t, prx::sim::application_t)
...
namespace prx
{
    ...
    namespace packages
    {
        ...
        namespace apc
        {
            ...
            apc_sim_no_sensing_t::apc_sim_no_sensing_t()
            {
                tf_broadcaster = NULL;
            ...
            void apc_sim_no_sensing_t::init(const parameter_reader_t * const reader)
            {
                Init_Finished = false;
                ...
                if(real_robot)
                {
                    real_robot_state = n.subscribe("/joint_states",4,&apc_sim_no_sensing_t::real_robot_state_callback,this);
            ...
            void apc_sim_no_sensing_t::real_robot_state_callback(const sensor_msgs::JointState& stateMsg)
            {
            ...
            void apc_sim_no_sensing_t::publish_state()
            {
            ...
            void apc_sim_no_sensing_t::frame(const ros::TimerEvent& event)
            {
                handle_key();
                // update_simulation();
                if( simulator_mode == 1 )
            ...
            void apc_sim_no_sensing_t::update_simulation()
            {
                if(!real_robot)
                {
            ...
            void apc_sim_no_sensing_t::place_object_callback(const prx_simulation::object_msg& objectMsg)
            {
                config_t new_config;
                foreach(movable_body_plant_t* plant, movable_bodies)
                {
            ...
            void apc_sim_no_sensing_t::check_object_collision(movable_body_plant_t* plant)
            {
            ...
            void apc_sim_no_sensing_t::reset_object_pose()
            {
            ...
            void apc_sim_no_sensing_t::set_selected_path(const std::string& path){}

            void apc_sim_no_sensing_t::create_and_send_robot_command()
            {
            ...
            void apc_sim_no_sensing_t::clear_plan()
            {
                has_goal = false;
            ...
            double apc_sim_no_sensing_t::create_robot_message(std::vector<trajectory_t*>& robot_trajs, std::vector<plan_t*>& robot_plans, std::vector<bool>& grasp_plan, int index)
            {
            ...
            double apc_sim_no_sensing_t::l2_norm(std::vector<double> const& u) 
            {
                double accum = 0.;
                for (double x : u) {
                    accum += x * x;
            ...
            bool apc_sim_no_sensing_t::start_servoing()
            {
                //LOGIC
            ...
            void apc_sim_no_sensing_t::send_robot_message(double duration)
            {
            ...
            std::string apc_sim_no_sensing_t::getStringFromEnum(automaton_states a_s)
            {
              switch (a_s)
              {
