...
#include "utilities/superball_center.hpp"
#include <pluginlib/class_list_macros.h> 

PLUGINLIB_EXPORT_CLASS( prx::packages::mapping_functions::superball_center_t, prx::util::mapping_function_t)
...
namespace prx
{
    ...
    {        
        ...
        {
            void superball_center_t::embed() const
            {
                ...
                for(unsigned i=0;i<3;i++)
                {
                    ...
                    {
                        val += get_preimage_index(start_index+j*13+i);
                    }
                    val/=6.0;
                    get_image_index(image_start_index+i) = val;
                }
                double temp = get_image_index(image_start_index+1);
                get_image_index(image_start_index+1) = -get_image_index(image_start_index+2);
                get_image_index(image_start_index+2) = temp;
            }

            void superball_center_t::invert() const
            {
                // PRX_INFO_S("DON'T EVEN TRY");
                // unsigned start_index = preimage_interval.first;
                // unsigned image_start_index = image_interval.first;
