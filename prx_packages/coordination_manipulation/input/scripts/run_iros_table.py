...
ALGORITHM =       [ "Naive",  "Batch_Random",  "Batch_MinConf", "Incremental_ShortestSelection", "Incremental_LongestSelection", "Incremental_RandomSelection", "Incremental_MinConfSelection"]
SCHEDULING_ALGORITHM = [ "naive: true", "batch: true", "batch: true", "incremental: true", "incremental: true", "incremental: true", "incremental: true" ]

SCHEDULING_ALGS = [ "RANDOM", "RANDOM", "MINCONF",  "INCREMENTAL", "INCREMENTAL",  "INCREMENTAL", "INCREMENTAL"  ]
SELECTION_ALGS =  [ "NONE",   "NONE",   "NONE",     "SHORTEST",    "LONGEST",       "RANDOM" ,    "MINCONF"]
BIAS_ALGS =       [ "JOINT",  "JOINT",  "JOINT",    "ANY",         "ANY",           "ANY",        "ANY"]

NODE_FILENAMES =  [ "NORMAL_NODES.txt",  "RANDOM_NODES.txt",  "NORMAL_NODES.txt", "NORMAL_NODES.txt", "NORMAL_NODES.txt", "RANDOM_NODES.txt", "NORMAL_NODES.txt"]
...
#NODE_FILENAMES =  [ "ALL_SHELF_LAUNCHES_8.txt",  "ALL_SHELF_LAUNCHES_8.txt",  "ALL_SHELF_LAUNCHES_8.txt", "ALL_SHELF_LAUNCHES_8.txt", "ALL_SHELF_LAUNCHES_8.txt", "ALL_SHELF_LAUNCHES_8.txt"]
K = [8]
...
if test == "iros_table2":
    ...
    CONSTRAINTS_DIRECTORY = "/prx_packages/coordination_manipulation/input/experiments/iros_table2/coordination_constraints/"
    
    sampling_poses_filename = "poses/all_poses_iros_table2.txt"
    final_poses_filename = "poses/all_poses_iros_table2.txt"
...
if test == "iros_shelf":
    ...
    LEFT_SAFE_STATE = "[1.5681561256801836989893673,-0.6933267886173464233934283,-0.8849957765343199334040492,1.9000335039215263677192524,-2.2107340224187499444497007,-0.8368545446250841290947164,0.9573491235931878007292539,0.0000000000000000000000000]"
    RIGHT_SAFE_STATE = "[-1.6812774859126198290226739,-0.5125644883535177553923745,1.0773961507954412653020881,1.8738453488311539452126908,1.9879612960600967497271085,-0.9987250827246928475133814,-0.8436528393489071131483570,0.0000000000000000000000000]"
    LEFT_ARM_DROPOFF = "[0.00000000,1.05000000,0.80000000,0.70710678,0.00000000,-0.70710678,0.00000000]"
    RIGHT_ARM_DROPOFF = "[0.00000000,-0.54500000,0.80000000,0.70710678,0.00000000,-0.70710678,0.00000000]"
    NUM_LEFT_PLANS = "12"
    NUM_RIGHT_PLANS = "12"
    ...
    for n in range(N):                
        ...
        for sched_alg in enumerate(ALGORITHM):
            ...
            for line in fin.readlines():
                ...
                line=re.sub('#NUMBER_OF_EXPERIMENTS#', NUMBER_OF_EXPERIMENTS, line)
                line=re.sub('#TOTAL_NEW_EXPERIMENTS#', TOTAL_NEW_EXPERIMENTS, line)
