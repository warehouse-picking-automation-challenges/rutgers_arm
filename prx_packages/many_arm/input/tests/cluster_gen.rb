#If we're doing processing on the cluster, we need to know how many processes
CLUSTER_PROCESSES = 500
$process = 0
...
def generate_bin( f, x, y, theta, i )
    ...
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/world_model/simulator/obstacles\">\n"
    f << "    bin" << i.to_s << ":\n"
    f << "      type: obstacle\n"
    f << "      geometries:\n"
    f << "        -\n"
...
def generate_baxter( f, x, y, theta, i )
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model/simulator/subsystems/baxter_" << i.to_s << "_left\">\n"
    f << "    template: \"plant\"\n"
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model/simulator/subsystems/baxter_" << i.to_s << "_right\">\n"
    f << "    template: \"plant\"\n"
    f << "    initial_state: [0.0,-1.34394,0.42624,2.61800,2.84100,2.09400,-0.06762,0]\n"
    f << "    hand: right\n"
    f << "    forward_transform_rotation: [" << a.to_s << "," << b.to_s << ",0," << c.to_s << "," << d.to_s << ",0,0,0,1]\n"
    ...
    #Transit
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/task_planner/planners/\" >\n"
...
def main()
    ...
    launchfile = expr + "_" + numstring + "_cluster_" + extstr + "search.launch"
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "\" file=\"$(find prx_input)/templates/spaces/space_types.yaml\"/>\n"
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model\" file=\"$(find manipulation)/input/simulation/manipulation_simulator.yaml\"/>\n"
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "\" file=\"$(find prx_input)/templates/planning_applications/single_query_application.yaml\"/>\n"
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/task_planner/\" file=\"$(find many_arm)/input/templates/mm_forward_tp.yaml\"/>\n\n"

    #Now, create the consumer mapping as well
    f << "  <!-- System Mappings -->\n"
    f << "  <rosparam ns=\"planning_" + $process.to_s + "\">\n"
    f << "    persistent: false\n"
    f << "    visualize: false\n"
    f << "    simulate: false\n"
    # f << "    system_mapping:\n"
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/plant\" file=\"$(find baxter)input/urdf/baxter_nomesh.yaml\"/>\n\n"
    ...
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/task_planner\">\n"
    ...
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/problems\">\n"
    f << "    benchmark_problem:\n"
    f << "      specification:\n"
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model/simulator\">\n"
    f << "    manipulator_names:\n"
    for i in 1 .. NUM_SYSTEMS
        f << "      -\n"
    ...
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/world_model/simulator/sensing\">\n"
    ...
    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/cup\" file=\"$(find manipulation)/input/simulation/plants/cups.yaml\"/>\n"
    f << "  <rosparam ns=\"planning_" + $process.to_s + "/cup\">\n"
    f << "    state_space:\n"
        ...
        for i in 1 .. NUM_SYSTEMS
            ...
            for j in 0 .. 11
                ...
                if bins[j] == 1
                    ...
                    f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model/simulator/subsystems/cup" << i.to_s << "_" << j.to_s << "\">\n"
                    f << "    template: \"cup\"\n"
    ...
    # = = = = = = =
        ...
        for j in 0 .. (nr_bins-1)
            if bins[j] == 1
                ...
                #Then output the stuff
                f << "  <rosparam command=\"load\" ns=\"planning_" + $process.to_s + "/world_model/simulator/subsystems/cup" << j.to_s << "\">\n"
                f << "    template: \"cup\"\n"
