    ...
    namespace packages
    {
        ...
        namespace rearrangement_manipulation
        {
            ...
            void manipulation_mp_specification_t::init(const util::parameter_reader_t* reader, const util::parameter_reader_t* template_reader)
            {
                ...
                is_builder = parameters::get_attribute_as<bool>("is_builder",reader,template_reader,false);
                transfer_mode = parameters::get_attribute_as<bool>("transfer_mode",reader,template_reader,false);
            }
            
            void manipulation_mp_specification_t::link_poses(std::vector<pose_t>* poses)
            {
                poses_set = poses;
            }

            void manipulation_mp_specification_t::link_poses_seeds(std::vector< std::pair<unsigned, sim::state_t*> >* poses)
            {
                this->poses = poses;
            }
            
            void manipulation_mp_specification_t::link_query_poses_seeds(std::vector< std::pair<unsigned, sim::state_t*> >* poses)
            {
