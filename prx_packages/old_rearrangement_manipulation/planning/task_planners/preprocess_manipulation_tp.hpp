        ...
        namespace rearrangement_manipulation
        {
            ...
            class preprocess_manipulation_tp_t : public plan::task_planner_t
            {
              ...
              protected:
                ...
                /** @brief A planning context only for the plannable cup.*/
                std::string pc_name_object_only;
                /** @brief A context for controlling all the objects. Active objects Plannable the movable cup.*/
                std::string pc_name_objects;
                /** @brief Plannable manipulator and object.*/
                std::string pc_name_manipulator_with_object;
                /** @brief Plannable manipulator and active object.*/
                std::string pc_name_grasp_checks;
                /** @brief The planning context for informing the transit graph.*/
                std::string pc_name_transit;
                /** @brief The planning context for informing the transfer graph.*/
                ...
                /** @brief The state space for the objects that will be used to inform the graph.*/
                const util::space_t* full_collision_objects_space;
                /** @brief The space for sampling random poses on the ground.*/
                util::space_t* stable_pose_space;
                /** @brief The space for sampling random poses on the space.*/
                util::space_t* extra_pose_space;

                //Helping variables for state points.
                sim::state_t* manip_state;
                sim::state_t* object_state;
                sim::state_t* object_moved_state;
                ...
                sim::state_t* stable_pose_state;
                 /** @brief The random sampled state for an extra pose on the environment.*/
                sim::state_t* extra_pose_state;
                /** @brief Helping variable for checking for collisions between poses while informing the graph.*/
                sim::state_t* full_collision_check_state;
                ...
                /** @brief Helping state point for grasped retracted configurations.*/
                sim::state_t* grasped_retracted_point; //manip and object state space
                ...
                /** @brief Helping state point for released configurations.*/
                sim::state_t* manip_grasping_point; //manip state space
                ...
                std::vector<double*> extra_pose_memory;

                /** @brief Temporary retract path storage. */
                sim::trajectory_t tmp_path;
                /** @brief Temporary reaching plans storage. */
                sim::plan_t transit_reaching_plan;
                sim::plan_t transfer_reaching_plan;
                /** @brief Temporary retract plans storage. */
                sim::plan_t transit_retract_plan;
                sim::plan_t transfer_retract_plan;                
              ...
              private:
                ...
                util::config_t tmp_start_config;
                util::config_t tmp_end_config;
                ...
                bool get_grasp_at_theta(sim::state_t* point, const sim::state_t* pose_state, int max_tries, double theta, bool full_motion = true);
                ...
                bool valid_move(sim::plan_t& plan, const sim::state_t* start, util::config_t& start_config, util::config_t& goal_config, bool set_grasping = false);

                bool valid_move(sim::plan_t& plan, const sim::state_t* start, const sim::state_t* goal, util::config_t& start_config, util::config_t& goal_config, bool set_grasping = false);
                ...
                std::string print(const std::set<unsigned>& set);
            };
        }
    }
}
