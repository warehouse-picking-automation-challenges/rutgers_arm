    ...
    namespace packages
    {
        namespace rearrangement_manipulation
        {
            ...
            void rearrangement_manipulation_tp_t::init(const parameter_reader_t* reader, const parameter_reader_t* template_reader)
            {
                ...
                std::string element = "manipulator_query";
                if( parameters::has_attribute(element, reader, template_reader) )
                {
                    ...
                    output_queries[manipulation_tp_name] = parameters::initialize_from_loader<query_t > ("prx_planning", reader, element, query_template_reader, "");
            ...
            {
                PRX_WARN_S("Get statistics for rearrangement_manipulation_tp_t is not implemented!");
            ...
            void rearrangement_manipulation_tp_t::setup()
            {
                ...
                transit_astar->link_valid_constraints(&transit_constraints);
                ...
                transfer_specification->object_space = collision_object_space;
                ...
                transfer_astar->link_valid_constraints(&transfer_constraints);
            ...
            bool rearrangement_manipulation_tp_t::execute()
            {
                ...
                //Build the initial state from the current positions for the cups given by the query.
                ...
                for( unsigned i = 0; i < poses->size(); ++i )
                {
                    ...
                    {                        
                        poses_set.push_back(pose_t(poses_set_length));
                ...
                for( unsigned i = seeds_start; i < poses_set_length; ++i )
                {
                    ...
                    {
                        transit_specification->add_seed(poses_set[i].ungrasped_set[j]);
...
//                        retract_plan.push_bach(manip_ctrl, retract_plan[0].duration);
...
//                model->use_context(old_context);
            ...
            bool rearrangement_manipulation_tp_t::is_valid_grasp(state_t* point)
            {
                ...
                if( validity_checker->is_valid(released_point) )
                {
                    ...
                    //TODO: if we add this functionality again we have to deal with this line.
...
//                }
                ...
                while( it != plan_parts.end() )
                {
                    ...
                    if( it2 != plan_parts.end() && (*it2)->object_id == id )
                    {
                        ...
                        if( (*it)->from_pose != to_pose )
                        {
                            ...
                            (*it)->reaching_point = manip_query->reaching_points[0];
                            (*it)->retracting_point = manip_query->retracting_points[0];
                            PRX_DEBUG_COLOR("compare points:  " << (*it)->reaching_point << " \n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->reaching_point - 2).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->reaching_point - 1).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->reaching_point).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->reaching_point + 1).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->reaching_point + 2).control, 5), PRX_TEXT_LIGHTGRAY);


                            PRX_DEBUG_COLOR("compare points:  " << (*it)->retracting_point << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point - 2).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point - 1).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point + 1).control, 5) << "\n"
                                            << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point + 2).control, 5), PRX_TEXT_BROWN);

                            for( int i = -6; i < 7; ++i )
                                PRX_DEBUG_COLOR(manip_control_space->print_point((*it)->plan.at((*it)->retracting_point + i).control, 5), PRX_TEXT_CYAN);
                            PRX_ASSERT(manip_control_space->equal_points((*it)->plan.at((*it)->reaching_point).control, (*it)->plan.at((*it)->reaching_point - 1).control));
                            PRX_ASSERT(manip_control_space->equal_points((*it)->plan.at((*it)->retracting_point).control, (*it)->plan.at((*it)->retracting_point + 1).control));
            ...
            {
                list<smoothing_info_t*>::iterator it = plan_parts.begin();
                list<smoothing_info_t*>::iterator it2 = it;

                transit_astar->set_minimum_conflict(false);
                transit_astar->set_shortest_path_flag(true);

                it2++;
                while( it2 != plan_parts.end() )
                {
                    //                    PRX_DEBUG_COLOR("IT1 size:" << (*it)->plan.size() << "    point:" << (*it)->retracting_point << "    IT2: size:" << (*it2)->plan.size() << "    point:" << (*it2)->reaching_point, PRX_TEXT_MAGENTA);
                    //                    PRX_DEBUG_COLOR("IT1 control at retracting -1: " << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point - 1).control, 5), PRX_TEXT_GREEN);
                    //                    PRX_DEBUG_COLOR("IT1 control at retracting   : " << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point).control, 5), PRX_TEXT_GREEN);
                    //                    PRX_DEBUG_COLOR("IT1 control at retracting +1: " << manip_control_space->print_point((*it)->plan.at((*it)->retracting_point + 1).control, 5), PRX_TEXT_GREEN);
                    //                    (*it)->trim_plan_after_retracting_point();
                    //                    unsigned sz = (*it)->plan.size();
                    //                    PRX_DEBUG_COLOR("IT1 control after trimming end-1: " << manip_control_space->print_point((*it)->plan[sz - 2].control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT1 control after trimming end  : " << manip_control_space->print_point((*it)->plan[sz - 1].control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching -3   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point - 3).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching -2   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point - 2).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching -1   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point - 1).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching      : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching +1   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point + 1).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching +2   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point + 2).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at reaching +3   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->reaching_point + 3).control, 5), PRX_TEXT_BLUE);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point -4   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 4).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point -3   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 3).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point -2   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 2).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point -1   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 1).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point      : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point +1   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 1).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point +2   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 2).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point +3   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 3).control, 5), PRX_TEXT_CYAN);
                    //                    PRX_DEBUG_COLOR("IT2 control at retracting_point +4   : " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 4).control, 5), PRX_TEXT_CYAN);
                    //                    (*it2)->trim_plan_before_reaching_point(manip_control_space);
                    //                    PRX_DEBUG_COLOR("IT2 control after trimming 0 : " << manip_control_space->print_point((*it2)->plan[0].control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 control after trimming 1 : " << manip_control_space->print_point((*it2)->plan[1].control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 control after trimming 2 : " << manip_control_space->print_point((*it2)->plan[2].control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after -3: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 3).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after -2: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 2).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after -1: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point - 1).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after  0: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after +1: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 1).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after +2: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 2).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT2 retracting_point after +3: " << manip_control_space->print_point((*it2)->plan.at((*it2)->retracting_point + 3).control, 5), PRX_TEXT_BROWN);
                    //                    PRX_DEBUG_COLOR("IT1 size:" << (*it)->plan.size() << "    point:" << (*it)->retracting_point << "    IT2: size:" << (*it2)->plan.size() << "    point:" << (*it2)->reaching_point, PRX_TEXT_MAGENTA);
                    ...
                    std::set<unsigned> poses;
                    detect_object_positions(poses, (*it2)->object_id, it2);
                    PRX_DEBUG_COLOR("Detected poses: " << print(poses), PRX_TEXT_MAGENTA);
                    transit_constraints = poses;
                    manip_query->clear();
                    manip_query->mode = manipulator_query_t::PRX_TRANSIT;
                    PRX_DEBUG_COLOR("Start: " << manip_state_space->print_point((*it)->get_retracting_point(), 5), PRX_TEXT_GREEN);
                    PRX_DEBUG_COLOR("End: " << manip_state_space->print_point((*it2)->get_reaching_point(), 5), PRX_TEXT_BROWN);
                    manip_query->copy_start((*it)->get_retracting_point());
                    dynamic_cast<multiple_goal_states_t*>(manip_query->get_goal())->copy_goal_state((*it2)->get_reaching_point());
                    manip_tp->link_query(manip_query);
                    manip_tp->resolve_query();
                    if( manip_query->plans[0]->size() != 0 )
                    {
                        (*it)->trim_plan_after_retracting_point();
                        //                        PRX_DEBUG_COLOR(manip_control_space->print_point((*it)->plan.back().control, 5), PRX_TEXT_GREEN);
                        //                        PRX_DEBUG_COLOR(manip_control_space->print_point(manip_query->plans[0]->at(0).control, 5), PRX_TEXT_BROWN);
                        (*it)->plan += (*manip_query->plans[0]);
                        (*it2)->trim_plan_before_reaching_point(manip_control_space);
                        PRX_DEBUG_COLOR(manip_control_space->print_point((*it2)->plan.at(0).control, 5), PRX_TEXT_GREEN);
                        PRX_DEBUG_COLOR(manip_control_space->print_point(manip_query->plans[0]->back().control, 5), PRX_TEXT_BROWN);
                        PRX_ASSERT(manip_control_space->equal_points((*it2)->plan.at(0).control, manip_query->plans[0]->back().control));
                ...
                //Find better path from the last pose back to safe state.
                transit_constraints.clear();
                transit_constraints.insert(target_poses_ids.begin(), target_poses_ids.end());

                manip_query->clear();
                manip_query->mode = manipulator_query_t::PRX_TRANSIT;
                PRX_DEBUG_COLOR("Start: " << manip_state_space->print_point((*it)->get_retracting_point(), 5), PRX_TEXT_GREEN);
                PRX_DEBUG_COLOR("End: " << manip_state_space->print_point(safe_state, 5), PRX_TEXT_BROWN);
                manip_query->copy_start((*it)->get_retracting_point());
                dynamic_cast<multiple_goal_states_t*>(manip_query->get_goal())->copy_goal_state(safe_state);
                manip_tp->link_query(manip_query);
                manip_tp->resolve_query();
                if( manip_query->plans[0]->size() != 0 )
                {
                ...
                //Find better path from the safe to the first pose.
                it = plan_parts.begin();
                transit_constraints.clear();
                transit_constraints.insert(initial_poses_ids.begin(), initial_poses_ids.end());

                manip_query->clear();
                manip_query->mode = manipulator_query_t::PRX_TRANSIT;
                PRX_DEBUG_COLOR("Start: " << manip_state_space->print_point(safe_state, 5), PRX_TEXT_GREEN);
                PRX_DEBUG_COLOR("End: " << manip_state_space->print_point((*it)->get_reaching_point(), 5), PRX_TEXT_BROWN);
                manip_query->copy_start(safe_state);
                dynamic_cast<multiple_goal_states_t*>(manip_query->get_goal())->copy_goal_state((*it)->get_reaching_point());
                manip_tp->link_query(manip_query);
                manip_tp->resolve_query();
                if( manip_query->plans[0]->size() != 0 )
                {
                    (*it)->trim_plan_before_reaching_point(manip_control_space);
                    (*manip_query->plans[0]) += (*it)->plan;
                    (*it)->plan = (*manip_query->plans[0]);
                }
                ...
                foreach(undirected_vertex_index_t v, boost::vertices(MPG_graph->graph))
                {
                    ...
                    for( unsigned i = 0; i < manip_query->plans.size(); ++i )
                    {
                        if( manip_query->plans[i] != NULL )
                        {
                            ...
                            new_info->retracted_point = manip_query->reaching_points[i];
                            ...
                            PRX_DEBUG_COLOR("compare points: \n" << manip_control_space->print_point(new_info->plan.at(new_info->retracted_point - 1).control, 5) << "\n"
                            ...
                            PRX_ASSERT(manip_control_space->equal_points(new_info->plan.at(new_info->retracted_point).control, new_info->plan.at(new_info->retracted_point - 1).control));
                            PRX_ASSERT(manip_control_space->equal_points(new_info->plan.at(new_info->retracted_point).control, poses_set[pose_index].retracted_set[i]));
                            PRX_ASSERT(new_info->plan.at(new_info->retracted_point).control->memory.back() == 0);
                        }
                        else
                        {
                    ...
                    for( unsigned j = i + 1; j < mpg_size; ++j )
                    {
                        ...
                        if( find_best_path(from_node->position_index, to_node->position_index, constraints, full_constraints, cost, grasp_from, grasp_to, transfer_index) )
                        {
                            ...
                            if( constraints.size() == 0 )
                            {
                                ...
                                edge->retracting_point = edge->plan.size() - grasping_poses[to_node->position_index][grasp_to]->retracted_point - 1;
                                PRX_DEBUG_COLOR("compare points: \n" << manip_control_space->print_point(edge->plan.at(edge->reaching_point - 1).control, 5) << "\n"
                                                << manip_control_space->print_point(edge->plan.at(edge->reaching_point).control, 5) << "\n"
                                                << manip_control_space->print_point(edge->plan.at(edge->reaching_point + 1).control, 5) << "\n"
                                                << manip_state_space->print_point(poses_set[from_node->position_index].retracted_set[grasp_from], 5), PRX_TEXT_LIGHTGRAY);

                                PRX_DEBUG_COLOR("compare points: \n" << manip_control_space->print_point(edge->plan.at(edge->retracting_point - 1).control, 5) << "\n"
                                                << manip_control_space->print_point(edge->plan.at(edge->retracting_point).control, 5) << "\n"
                                                << manip_control_space->print_point(edge->plan.at(edge->retracting_point + 1).control, 5) << "\n"
                                                << manip_state_space->print_point(poses_set[to_node->position_index].retracted_set[grasp_to], 5), PRX_TEXT_BROWN);
                                PRX_ASSERT(edge->plan.at(edge->reaching_point).control->memory.back() == 0);
                                PRX_ASSERT(edge->plan.at(edge->retracting_point).control->memory.back() == 0);
                                PRX_ASSERT(manip_control_space->equal_points(edge->plan.at(edge->reaching_point).control, edge->plan.at(edge->reaching_point - 1).control));
                                PRX_ASSERT(manip_control_space->equal_points(edge->plan.at(edge->retracting_point).control, edge->plan.at(edge->retracting_point + 1).control));
                ...
                if( specs->astar->solve(v_initial, v_target) )
                {
                    ...
                    for( int i = 0; i < (int)path_vertices.size() - 1; ++i )
                    {
                        ...
                        if( edge->has_motion )
                        {
                            ...
                            edge->get_plan(part->plan, path_vertices[i]);
                            edge->get_connecting_points(part->reaching_point, part->retracting_point, path_vertices[i]);
                            PRX_DEBUG_COLOR("Control at reaching_point -1: " << manip_control_space->print_point(part->plan.at(part->reaching_point - 1).control, 5), PRX_TEXT_GREEN);
                            PRX_DEBUG_COLOR("Control at reaching_point   : " << manip_control_space->print_point(part->plan.at(part->reaching_point).control, 5), PRX_TEXT_GREEN);
                            PRX_DEBUG_COLOR("Control at reaching_point +1: " << manip_control_space->print_point(part->plan.at(part->reaching_point + 1).control, 5), PRX_TEXT_GREEN);
                            PRX_DEBUG_COLOR("Control at retracting -1: " << manip_control_space->print_point(part->plan.at(part->retracting_point - 1).control, 5), PRX_TEXT_CYAN);
                            PRX_DEBUG_COLOR("Control at retracting   : " << manip_control_space->print_point(part->plan.at(part->retracting_point).control, 5), PRX_TEXT_CYAN);
                            PRX_DEBUG_COLOR("Control at retracting +1: " << manip_control_space->print_point(part->plan.at(part->retracting_point + 1).control, 5), PRX_TEXT_CYAN);
                            PRX_DEBUG_COLOR("Control at edge retracting -1: " << manip_control_space->print_point(part->plan.at(edge->retracting_point - 1).control, 5), PRX_TEXT_LIGHTGRAY);
                            PRX_DEBUG_COLOR("Control at edge retracting   : " << manip_control_space->print_point(part->plan.at(edge->retracting_point).control, 5), PRX_TEXT_LIGHTGRAY);
                            PRX_DEBUG_COLOR("Control at edge retracting +1: " << manip_control_space->print_point(part->plan.at(edge->retracting_point + 1).control, 5), PRX_TEXT_LIGHTGRAY);
                            PRX_ASSERT(manip_control_space->equal_points(part->plan.at(part->reaching_point - 1).control, part->plan.at(part->reaching_point).control));
                            PRX_ASSERT(manip_control_space->equal_points(part->plan.at(part->retracting_point).control, part->plan.at(part->retracting_point + 1).control));
            ...
            void rearrangement_manipulation_tp_t::compose_final_plan()
            {
                ...
                if( !validate_full_path() )
                    PRX_ASSERT_MSG(false, "Not valid final path");
            ...
            void rearrangement_manipulation_tp_t::solve_path(const super_node_t& node, std::vector< unsigned >& start_arrangement, const std::vector< unsigned > goal_arrangement)
            {
                ...
                for( unsigned i = 0; i < plan_vertices.size(); ++i )
                {
                    ...
                    edge->get_connecting_points(part->reaching_point, part->retracting_point, plan_vertices[i].first);
                    PRX_DEBUG_COLOR("Control at reaching_point -1: " << manip_control_space->print_point(part->plan.at(part->reaching_point - 1).control, 5), PRX_TEXT_GREEN);
                    PRX_DEBUG_COLOR("Control at reaching_point   : " << manip_control_space->print_point(part->plan.at(part->reaching_point).control, 5), PRX_TEXT_GREEN);
                    PRX_DEBUG_COLOR("Control at reaching_point +1: " << manip_control_space->print_point(part->plan.at(part->reaching_point + 1).control, 5), PRX_TEXT_GREEN);
                    PRX_DEBUG_COLOR("Control at retracting -1: " << manip_control_space->print_point(part->plan.at(part->retracting_point - 1).control, 5), PRX_TEXT_CYAN);
                    ...
                    PRX_DEBUG_COLOR(part->print(), PRX_TEXT_MAGENTA); // (31 + (i % 7)));
                }
            ...
            std::string rearrangement_manipulation_tp_t::print(const std::vector<double>& arrangement)
            {
            ...
            std::string rearrangement_manipulation_tp_t::print(const std::deque<unsigned>& arrangement)
            {
                std::stringstream output(std::stringstream::out);
            ...
            std::string rearrangement_manipulation_tp_t::print(const std::set<unsigned>& constraints)
            {
                std::stringstream output(std::stringstream::out);
