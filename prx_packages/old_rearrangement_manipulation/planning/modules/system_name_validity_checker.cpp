    ...
    namespace packages
    {
        ...
        namespace rearrangement_manipulation
        {
            ...
            bool system_name_validity_checker_t::is_valid(const state_t* point)
            {
                ...
                foreach(const collision_pair_t& cp, world_model->get_colliding_bodies(point)->get_body_pairs())
                {
            ...
            bool system_name_validity_checker_t::has_constraints(std::set<unsigned>& constraints, const trajectory_t& path)
            {

                foreach(state_t* state, path)
                {
                    has_constraints(constraints,state);
                }
                return constraints.size() > 0;
            }

            bool system_name_validity_checker_t::has_constraints(std::set<unsigned>& constraints, const sim::state_t* state)
            {
                collision_list_t* cb = world_model->get_colliding_bodies(state);

                //                    PRX_DEBUG_COLOR("OBjects in collision: " << cb->size(), PRX_TEXT_GREEN);

                foreach(const collision_pair_t& cp, cb->get_body_pairs())
                {
                    //                        PRX_DEBUG_COLOR("Collision : " << cp.first << " - " << cp.second, PRX_TEXT_CYAN);
                    if( cp.first != end_effector && cp.second != end_effector )
                    {
                        if( ignore_list == NULL || !(ignore_list->pair_in_list(cp.first, cp.second)) )
                        {
                            //                                if( object_map.find(cp.first) != object_map.end() )
                            //                                {
                            //                                    PRX_DEBUG_COLOR("Collision : " << cp.first << " - " << cp.second << " : " << object_map[cp.first], PRX_TEXT_CYAN);
                            //                                }
                            //
                            //                                if( object_map.find(cp.second) != object_map.end() )
                            //                                {
                            //                                    PRX_DEBUG_COLOR("Collision : " << cp.first << " - " << cp.second << " : " << object_map[cp.second], PRX_TEXT_GREEN);
                            //                                }
                            //                                PRX_DEBUG_COLOR("Collision : " << cp.first << " - " << cp.second, PRX_TEXT_RED);
                            if( object_map.find(cp.first) != object_map.end() )
                            {
                                constraints.insert(object_map[cp.first]);
                            }

                            if( object_map.find(cp.second) != object_map.end() )
                            {
                                constraints.insert(object_map[cp.second]);
                            }
                        }
                    }
                }
            ...
            std::string system_name_validity_checker_t::collides_with()
            {
                std::string obj_name = object_name + "/body";
                ...
                {                    
                    ...
                    {                        
                        if( ignore_list == NULL || !(ignore_list->pair_in_list(cp.first, cp.second)) )
                        {
//                            PRX_DEBUG_COLOR("collision pair: " << cp.first << " - " << cp.second, PRX_TEXT_RED);
                            if( cp.first == obj_name )
                            {
                                return cp.second;
                            }

                            if( cp.second == obj_name )
                            {
                                return cp.first;
                            }
                        }
            ...
            void system_name_validity_checker_t::link_map(util::hash_t<std::string, unsigned>& object_map)
            {
