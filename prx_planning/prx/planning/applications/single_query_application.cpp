...
namespace prx
{
    ...
    namespace plan
    {
        ...
        {
            // PRX_DEBUG_COLOR("1", PRX_TEXT_CYAN);
            this->root_task->link_specification(root_specifications[0]);
            // PRX_DEBUG_COLOR("2", PRX_TEXT_CYAN);
            this->root_task->setup();
            // PRX_DEBUG_COLOR("3", PRX_TEXT_CYAN);
            this->root_task->link_query(root_queries[0]);
            // PRX_DEBUG_COLOR("4", PRX_TEXT_CYAN);
            ...
            if( interruptable )
            {
                ...
                // PRX_DEBUG_COLOR("5", PRX_TEXT_CYAN);
                root_specifications[0]->get_stopping_criterion()->link_interruption_criteria(interruption_criteria);
                ...
                while( !done )
                {
                    try
                    {
                        // PRX_DEBUG_COLOR("6", PRX_TEXT_CYAN);
            ...
            const statistics_t* stats = root_task->get_statistics();
            ...
            if( simulate )
            {
                if( dynamic_cast<multi_planner_query_t*>(root_queries[0]) != NULL )
                {
                    ...
                    {
                        ((comm::planning_comm_t*)comm::plan_comm)->publish_plan(consumer_path, ((motion_query_t*)(query))->plan);
                    }
                }
                else
                {
                    ((comm::planning_comm_t*)comm::plan_comm)->publish_plan(consumer_path, dynamic_cast<motion_query_t*>(root_queries[0])->plan);
                }
            }
