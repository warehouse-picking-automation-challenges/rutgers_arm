    ...
    namespace plan
    {
        ...
        void planning_application_t::init(const parameter_reader_t* reader)
        {
            ...
            if( simulate )
            {
                ...
                PRX_DEBUG_COLOR("This planning node will talk to simulation node " << simulation_node_name, PRX_TEXT_RED);
            }
            ...
            double num_parallel_models = reader->get_attribute_as<double>("parallel_models",0);
            for(unsigned i=0;i<num_parallel_models;i++)
            {
                parallel_models.push_back(reader->create_from_loader<world_model_t > ("world_model", "prx_planning"));
                reader->initialize(parallel_models.back(), "world_model");
            }
            ...
            if( simulate && reader->has_attribute("consumer_mapping") )
            {
                ...
                foreach(const parameter_reader_t* item_reader, consumer_mapping)
                {
                    ...
                    PRX_DEBUG_COLOR("Paired consumer: " << pair[0] << " with space name: " << pair[1], PRX_TEXT_LIGHTGRAY);
                }
            }
            ...
            if(!parameters::has_attribute("task_planner/specification", reader,NULL))
                PRX_FATAL_S("Planning application has a root task planner without a specification!" );

            root_specifications.push_back(parameters::initialize_from_loader< specification_t >( "prx_planning", reader, "task_planner/specification", NULL, "" ) );

            //========================================
            // Initialize the query for this planner (if it has one).
            //========================================
            if(parameters::has_attribute("task_planner/query", reader,NULL))
            {
                root_queries.push_back(parameters::initialize_from_loader<query_t > ("prx_planning", reader, "task_planner/query", NULL, ""));
            }
        ...
        void planning_application_t::initialize_spaces(std::string consumer_path)
        {
            ...
            foreach(interval_msg msg, state_interval_msg)
            {
                if( msg.lower != msg.upper )
                {
                    ...
                    else
                    {
                        ...
                        interval_pair_t new_pair = std::make_pair(sim_interval, wm_interval);
            ...
            foreach(interval_msg msg, control_interval_msg)
            {
                ...
                {
                    system_ptr_t wm_sys = model->get_split_system(wm_sys_path);
                    const space_t* wm_control_space = model->get_control_space();
                    const space_t* wm_sys_space = wm_sys.get()->get_control_space();

                    // Create wm's interval
                    interval_t wm_interval;
                    wm_interval = wm_control_space->get_subspace(wm_sys_space);
                    interval_pair_t new_pair = std::make_pair(sim_interval, wm_interval);
