...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void world_model_t::init(const parameter_reader_t* reader, const parameter_reader_t* template_reader)
        {
            ...
            phys_based_sim = (sim_type == "ode_simulator") || (sim_type == "bullet_simulator" || (sim_type=="ntrt_simulator"));
            ...
            if( world_model_reader->has_attribute("planning_contexts") )
            {
                ...
                foreach(const parameter_reader_t::reader_map_t::value_type key_value, contexts_map)
                {
                    ...
                    PRX_DEBUG_COLOR("Creating a planning context: "<<key_value.first<<" Embedding?: "<<use_embedded_space, PRX_TEXT_CYAN);
                    ...
                    {
                        PRX_DEBUG_COLOR("-- Examining: " << system_pointer->get_pathname(), PRX_TEXT_LIGHTGRAY );
        ...
        void world_model_t::convert_plan(sim::plan_t& output_plan, const util::space_t* output_space, const sim::plan_t& input_plan, const util::space_t* input_space)
        {
        ...
        bool world_model_t::valid_state(const state_t* state, bool print_collisions)
        {
            selected_state_space->copy_from_point(state);
            // if(!selected_state_space->satisfies_bounds(state,false))
            // {
            //    return false;
            // }
            if (print_collisions)
            {
                foreach(collision_pair_t p, get_colliding_bodies()->get_body_pairs())
                {
                    PRX_INFO_S(p.first<<" "<<p.second);
                }
            }
...
//            selected_state_space->copy_from_point(goal);
        ...
        void world_model_t::init_collision_list(const parameter_reader_t * const reader)
        {
            ...
            PRX_DEBUG_COLOR(" init_collision_list has been called ", PRX_TEXT_GREEN);
        ...
        void world_model_t::add_system_system_in_collision_list(const std::string& system1, const std::string& system2, const vector_collision_list_t* black_list, hash_t<std::string, plant_t*>& plants)
        {
            ...
            {
                PRX_DEBUG_COLOR("Added plant: " << system1 << " vs. plant: " << system2, PRX_TEXT_CYAN);
