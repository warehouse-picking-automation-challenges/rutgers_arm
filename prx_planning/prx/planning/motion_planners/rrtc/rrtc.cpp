...
namespace prx
{
    ...
    namespace plan
    {
        ...
        void rrtc_t::link_specification(specification_t* new_spec)
        {
            motion_planner_t::link_specification(new_spec);
            //Make sure we explicitly relink the specifications here to ensure the proper spaces propagate
            rrt[0]->input_specification->link_spaces( new_spec->state_space, new_spec->control_space );
            rrt[1]->input_specification->link_spaces( new_spec->state_space, new_spec->control_space );
        }
        ...
        void rrtc_t::link_query(query_t* new_query)
        {
            ...
            rrt[0]->input_specification->link_spaces(state_space, control_space);
            ...
            rrt[0]->input_query->copy_start(start);
            ...
            rrt[1]->input_specification->link_spaces(state_space, control_space);
            //Tree 1 starts at the goal state instead
            unsigned goals_size;
            const space_point_t* goal = input_query->get_goal()->get_first_goal_point();
            rrt[1]->input_specification->clear_seeds();
            rrt[1]->input_specification->add_seed(goal);
            rrt[1]->input_query->copy_start(goal);

            //Try to be safe in case someone has changed the statespace on us
            rrt[1]->input_specification->metric->link_space(input_query->state_space);

            tree_goals[0]->copy_goal_state(goal);
            tree_goals[1]->copy_goal_state(start);
            ...
            //For debugging purposes, we should ensure that we provided a valid query
            if(!validity_checker->is_valid(start))
            {
                PRX_ERROR_S("Error:  Provided START state is invalid.  RRTc cannot run.");
            }
            if(!validity_checker->is_valid(goal))
            {
                PRX_ERROR_S("Error:  Provided GOAL state is invalid.  RRTc cannot run.");
            }
        }
        ...
        {
            built_tree = true;
            //PRX_DEBUG_COLOR("EXECUTE OF THE RRTc NOW!", PRX_TEXT_RED);
            return motion_planner_t::execute();
        }
        ...
        void rrtc_t::step()
        {
            ...
            //PRX_DEBUG_COLOR("Nodes: [" << rrt[0]->point_number << "] : [" << rrt[1]->point_number << "]", PRX_TEXT_GREEN);
            ...
            tree_goals[tree_b]->copy_goal_state(new_state); //Tree B has the appropriate goal now
            ...
            while( node_added )
            {
                ...
                if( rrt[tree_b]->point_number == last_num )
                {
                    ...
                    if( rrt[tree_b]->point_number > 0 )
                    {
                        ...
                        {
                            //PRX_DEBUG_COLOR("RRTc success!", PRX_TEXT_GREEN);
                            trees_met = true;
                            //And if they have met, update the goals appropriately
                            tree_goals[tree_a]->copy_goal_state(reached_state); //Now Tree A has the appropriate goal
                        }
                    }
                }
        ...
        {
            //PRX_DEBUG_COLOR("Success check: " << trees_met, PRX_TEXT_MAGENTA);
            return trees_met;
        }
        ...
        void rrtc_t::resolve_query()
        {
            ...
            if( !built_tree )
            {
                PRX_WARN_S("Resolving query without having run execute, executing the planner first...");
                execute();
            }
            ...
            if( !succeeded() )
            {
                PRX_WARN_S("\n\n\n = Resolving query when the trees have not connected.  No path to return... = \n\n");
                return;
            }
            ...
            unsigned goals_size;
            std::vector<space_point_t*> goals = rrt[1]->input_query->get_goal()->get_goal_points(goals_size);
            std::vector<tree_vertex_index_t> v_goals;
            v_goals.clear();
            
            for(unsigned i=0; i<goals_size; ++i)
            {
                tree_vertex_index_t source = rrt[1]->nearest_vertex(goals[i]);
