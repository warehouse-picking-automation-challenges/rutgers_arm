...
namespace prx
{
    ...
    namespace plan
    {
        ...
        motion_planner_t::motion_planner_t()
        {
            ...
            input_query = NULL;
            statistics = NULL;
            deserialize_flag = serialize_flag = deserialize_folder_flag = expand_flag =false;
        }
        ...
        void motion_planner_t::init(const parameter_reader_t* reader, const parameter_reader_t* template_reader)
        {
            ...
            if( has_attribute("deserialize_folder", reader, template_reader) )
            {
                PRX_DEBUG_S("Deserializing all files in folder");
                deserialization_folder = get_attribute_as<std::string > ("deserialize_folder", reader, template_reader);
                
                deserialize_flag = false;
                deserialize_folder_flag = true;
                connect_flag = true;
            ...
            {
                PRX_DEBUG_S("Deserialization variable set");
                deserialization_file = get_attribute_as<std::string > ("deserialize_file", reader, template_reader);
                deserialize_flag = true;

            }

            if(has_attribute("expand", reader, template_reader))
            {
                expand_flag = get_attribute_as<bool> ("expand", reader, template_reader);
                if (expand_flag)
                {
                    connect_flag = false;
                }
