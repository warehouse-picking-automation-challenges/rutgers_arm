    ...
    namespace plan
    {
        ...
        bool single_shot_planner_t::execute()
        {
            ...
            while( !done )
            {
                ...
                //TODO: WHAT IS THIS PART OF THE CODE SUPPOSED TO MEAN ANYWAY?  NEED TO CLEAN THIS CLASS UP BIG TIME!
                //Shouldn't this part of the code execute when we just want to deserialize and resolve a query?
                // done = true;
                // planners[planner_names[0]]->link_query(output_queries[planner_names[0]]);
                // planners[planner_names[0]]->resolve_query();
                // if( serialize_flag )
        ...
        {
            specification = dynamic_cast<single_shot_specification_t*>(in_specification);
        }

        void single_shot_planner_t::link_query(query_t* in_query)
        {
            //this will overwrite any query read from input
            query = dynamic_cast<single_shot_query_t*>(in_query);
        ...
        {
            // PRX_PRINT("In the SINGLE SHOT RESOLVE QUERY! : " << query, PRX_TEXT_RED);
            //Alright, how do we do this conversion...?
            query->plan.link_control_space( model->get_full_control_space() );
            
            // PRX_PRINT("Low-level plan: " << sub_query->plan.print(), PRX_TEXT_GREEN);
            // PRX_PRINT("Full control space: " << model->get_full_control_space() << " : " << model->get_full_control_space()->get_dimension(), PRX_TEXT_BLUE );
            // PRX_PRINT("Sub-control space: " << model->get_control_space() << " : " << model->get_control_space()->get_dimension(), PRX_TEXT_CYAN );
            
            model->convert_plan( query->plan, model->get_full_control_space(), sub_query->plan, model->get_control_space() );

            // PRX_PRINT("Converted Plan: " << query->plan.print(), PRX_TEXT_GREEN);
            
            query->path = sub_query->path;
            // query->plan = sub_query->plan;
        }
