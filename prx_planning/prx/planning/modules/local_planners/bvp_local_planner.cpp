...
namespace prx
{
    ...
    namespace plan
    {
        ...
        bvp_local_planner_t::bvp_local_planner_t(double prop_length)
        {
            max_prop_length = prop_length;
        }
        ...
        void bvp_local_planner_t::steer(const state_t* start, const state_t* goal, plan_t& plan, state_t* result, bool connect)
        {
            ...
            //DEBUG: We need to test the accuracy of plan propagation somehow... might as well do it here;
            // bool close = true;
            // for( unsigned i=7; i<result->memory.size() && close; ++i )
            // {
            //     if( fabs( result->memory[i] - goal->memory[i] ) > 0.005 )
            //         close = false;
            // }
            // if( !close )
            // {
            //     PRX_DEBUG_COLOR("Something went wrong, steered-to state has error:", PRX_TEXT_RED);
            //     PRX_DEBUG_COLOR("Expected : " << world_model->get_state_space()->print_point( goal, 6 ), PRX_TEXT_CYAN );
            //     PRX_DEBUG_COLOR("Resulting: " << world_model->get_state_space()->print_point( result, 6 ), PRX_TEXT_CYAN );
            // }
