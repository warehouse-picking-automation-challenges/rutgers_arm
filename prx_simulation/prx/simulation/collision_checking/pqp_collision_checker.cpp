    ...
    namespace sim
    {
        ...
        void pqp_collision_checker_t::create_model(const std::string& name, const geometry_t& geometry)
        {
            ...
            for( int i = 0; i < trimesh.get_faces_size(); ++i )
            {
                ...
                first[0] = (PQP_REAL)vertex[0];
                first[1] = (PQP_REAL)vertex[1];
                first[2] = (PQP_REAL)vertex[2];

                trimesh.get_vertex_at(face.get_index2(), &vertex);
                radius = PRX_MAXIMUM(radius, sqrt(vertex[0] * vertex[0] + vertex[1] * vertex[1] + vertex[2] * vertex[2]));
                second[0] = (PQP_REAL)vertex[0];
                second[1] = (PQP_REAL)vertex[1];
                second[2] = (PQP_REAL)vertex[2];

                trimesh.get_vertex_at(face.get_index3(), &vertex);
                radius = PRX_MAXIMUM(radius, sqrt(vertex[0] * vertex[0] + vertex[1] * vertex[1] + vertex[2] * vertex[2]));
                third[0] = (PQP_REAL)vertex[0];
                third[1] = (PQP_REAL)vertex[1];
                third[2] = (PQP_REAL)vertex[2];
