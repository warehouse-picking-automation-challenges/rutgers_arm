...
namespace prx
{
    ...
    namespace sim
    {
        util::hash_t< std::string, util::parameter_reader_t* > controller_t::template_readers_hash;
        double controller_t::clock_time = 0;
        unsigned controller_t::clock_calls = 0;
        ...
        controller_t::~controller_t()
        {
            ...
            //Need to clean out our template readers
            foreach( parameter_reader_t* reader, template_readers_hash | boost::adaptors::map_values )
            {
                delete reader;
            }
        }
            ...
            if( parameters::has_attribute("subsystems", reader, template_reader) )
            {
                ...
                foreach(const parameter_reader_t::reader_map_t::value_type key_value, subsystem_map)
                {
                    ...
                    if( key_value.second->has_attribute("template") )
                    {
                        ...
                        //If we have constructed this parameter reader before
                        if( template_readers_hash.find( template_name ) != template_readers_hash.end() )
                        {
                            //Simply use it
                            subsystems_template_reader = template_readers_hash[ template_name ];
                        }
                        else
                        {
                            //Otherwise, we have to build it and store it in the hash
                            stats_clock.reset();
                            template_readers_hash[ template_name ] = new parameter_reader_t(ros::this_node::getName() + "/" + template_name, global_storage);
                            subsystems_template_reader = template_readers_hash[ template_name ];
                            clock_time += stats_clock.measure();
                            ++clock_calls;
                        }
