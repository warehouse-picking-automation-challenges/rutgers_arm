...
//            construct_spaces();
        ...
        void simulator_t::update_collision_info()
        {
            for(auto& entry: plants)
            {
                entry.second->clear_collision_infos();
            }
            controller_t::update_collision_info();
        }
        ...
        double simulator_t::time_pqp = 0;
        double simulator_t::time_ppqp = 0;
        int simulator_t::total_trajs = 0;
        int simulator_t::total_iters = 0;
        bool simulator_t::traj_in_collision(trajectory_t& in_traj)
        {
            total_trajs++;
            if(dynamic_cast<parallel_collision_checker_t*>(collision_checker)==nullptr)
            {
               // std::cout << "collision_checker in traj_in_collision" << std::endl;
                double this_time_pqp = 0;
                double this_time_ppqp = 0;
                ...
                this_time_pqp = (end - start)/(double) CLOCKS_PER_SEC;

                total_iters += itr;
                time_pqp += this_time_pqp;
            #ifndef NDEBUG
                std::cout.setf(std::ios::boolalpha);
                std::cout << "iter = " << itr << ", avg iter: " << total_iters/total_trajs
                << ", time: " <<  this_time_pqp 
                << ", total " << time_pqp << ", avg = " << time_pqp/total_trajs 
                << ", result: " << collision1 << std::endl;
            #endif
                return collision;
            }
            else
            {
                //std::cout << "parallel_collision_checker in traj_in_collision" << std::endl;
                //std::cout << "collision_checker in traj_in_collision" << std::endl;
                double this_time_pqp = 0;
                double this_time_ppqp = 0;
                bool collision1 = false;
            ...
            #ifndef NDEBUG
                std::cout.setf(std::ios::boolalpha);
                std::cout << "iter = " << itr << ", time: " <<  this_time_pqp 
                << ", total " << time_pqp << ", avg = " << total_iters/total_trajs 
                << ", result: " << collision1 << std::endl;
            #endif
                return collision1;
/*
                ...
                collision2 = collision_checker->in_collision();
                //if(collision)
                std::clock_t end2 = std::clock();
                this_time_ppqp = (end2 - start)/(double) CLOCKS_PER_SEC;
                time_ppqp += this_time_ppqp;

                std::cout << "trajs:" << in_traj.length() << "time: " <<  this_time_ppqp
                <<", total " << time_ppqp << ", avg:" << time_ppqp/total_trajs 
                << ", result: " << collision2 << std::endl;*/

/*                std::cout << "iter = " << itr << ", time: " <<  this_time_pqp << " " << this_time_ppqp
                <<", total " << time_pqp << " " << time_ppqp << "result: " << collision1 << " "
                << collision2 << std::endl;*/
 //               return collision2;
            }
        }
