...
namespace prx {
    ...
    namespace sim {
        namespace comm {
            ...
            void visualization_comm_t::send_rigid_bodies()
            {
                if(app->is_visualizing())
                {

                    ros::ServiceClient client = node.serviceClient<describe_geometries_srv > ("prx/visualization/rigid_bodies");
                    describe_geometries_srv srv;

                    // // srv.request.rigid_body_array
                    //  rigid_body_info_msg
                    //     string system_name
                    ...
                    geom_map_t map;
                    // app->get_simulator()->update_phys_geoms(map);
                    // for(auto body:map)
                    // {
                    //     PRX_INFO_S(body.first);
                    // }
                    // map.clear();
                    for(auto obs:app->get_simulator()->get_obstacles())
                    {
                        obs.second->update_phys_geoms(map);
                    }
                    for(auto body:map)
                    {
                        prx_utilities::rigid_body_info_msg msg;
                        msg.system_name = ros::this_node::getName().substr(1,ros::this_node::getName().length())+"/"+reverse_split_path(body.first).first;
                        msg.rigid_body_name = reverse_split_path(body.first).second;
                        msg.material_name = "silver";
                        msg.geometry_type = body.second.get_type();
                        msg.geometry_params = *body.second.get_info();
                        if(msg.geometry_type == geometry_type::PRX_HEIGHTMAP || msg.geometry_type == geometry_type::PRX_MESH)
                            msg.mesh_name = body.second.get_mesh();
                        srv.request.rigid_body_array.push_back(msg);

                        // PRX_INFO_S(ros::this_node::getName().substr(1,ros::this_node::getName().length())+"/"+body.first);
                    }
                    client.waitForExistence(ros::Duration(10));
                    if (!client.call(srv)) {
                        PRX_ERROR_S("Failed to send rigid bodies!");
                    }
