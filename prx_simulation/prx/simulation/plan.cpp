...
namespace prx
{
    ...
    namespace sim
    {
        ...
        void plan_t::replace(int start_index, int end_index, const plan_t& input_plan)
        {
            PRX_ASSERT(control_space != NULL);
            plan_t new_plan(control_space);
            ...
            {
                new_plan.copy_onto_back(steps[i].control, steps[i].duration);
            }
            
            foreach(plan_step_t step, input_plan.steps)
            {
                new_plan.copy_onto_back(step.control, step.duration);
            }

            for(unsigned i = end_index; i < this->size(); ++i)
            {
                new_plan.copy_onto_back(steps[i].control, steps[i].duration);
            }
