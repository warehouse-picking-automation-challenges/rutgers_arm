...
namespace prx
{
    ...
    namespace sim
    {
        ...
        {
            // PRX_PRINT("Config Sensor Update Data", PRX_TEXT_RED);
            // PRX_PRINT("The number of sensed plants: " << sensed_plants.size(), PRX_TEXT_LIGHTGRAY);
            unsigned index = 0;
        ...
        const util::config_list_t& config_sensor_t::get_plant_configs()
        {
            //PRX_DEBUG_COLOR("plant configs"<< plant_configs, PRX_TEXT_LIGHTGRAY);
            return plant_configs;
        }

        const util::config_list_t& config_sensor_t::get_obstacle_configs()
        {
